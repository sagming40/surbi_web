...
| 카테고리 | 기획 |
| 파일형태 | 문서 |
| 버전 | v3.2 Final (브랜치 전략 확정 · 로그인 방식 확정) |
| 생성일 | 2026년 6월 18일 |
| 최종 수정 | 2026년 6월 28일 |
| 담당자 | 사공민규 |
...
> ⚠️ **API 연동 전 백엔드 API 명세 확정(Task 4-2) 필요 — With. 최민수**

---

### ⏳ Task 4-1 · Mock 데이터 구조 정의 (백엔드 대기 중 개발용)

> 💡 **왜 Mock 데이터가 필요한가?**
> 백엔드 API가 아직 완성되지 않았을 때도 UI 개발을 계속할 수 있음.
> 백엔드 API 완성 이후 Mock → 실제 API로 교체 예정.
...
class MockData {
  ...
  static ScoreResult scoreResult() => ScoreResult(
    totalScore: 78.5,
    predictedSales: 4200000,
    closureRiskPct: 23.1,
    shapFactors: [
      ShapFactor(name: '유동인구 지수', value: 18.2, maxScore: 20),
      ShapFactor(name: '업종 소비력',   value: 15.1, maxScore: 20),
      ShapFactor(name: '경쟁 강도',     value: -4.5, maxScore: 15),
...
- [ ]  `services/mock_data.dart` 작성
- [ ]  각 Provider에서 Mock 데이터 우선 반환하도록 설정
- [ ]  API 연동 완료 시 Mock → 실제 API로 교체
...
| 3-1 | [Step 1] 지역 / 카테고리 선택 UI | 화면 | P1 | ⏳ 예정 |
...
② Task 4-1 Mock 데이터 정의
   → 이것만 있으면 백엔드 없이 EPIC 3 화면 개발 바로 시작 가능

③ Task 2-1 Firebase 프로젝트 생성 (백엔드 의존 없이 혼자 진행 가능)

④ Task 4-2 API 명세 백엔드와 합의
   → 인증 엔드포인트 일정 포함 (/auth/kakao, /auth/naver)
   → models/ 코드 확정

⑤ EPIC 3 화면 개발 (Mock 데이터 기반, Step 1부터)
...
- scores 테이블 전달 방식 미정 → ML · 백엔드 · DB 협의 필요
...
*FRONT-END WORKFLOW v3.2 Final · 사공민규 · 최종 수정: 2026.06.28*
