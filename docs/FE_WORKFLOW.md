...
| 순위 | 작업 | 상태 | 비고 |
| --- | --- | --- | --- |
| **1** | **Step 1·2·3 통합 화면** (`feature/frontend-unified-map`) | 🔄 **Phase 1·2-A·2-B 완료** | 다음은 2-C(폴리곤 클릭). 아래 "통합 화면 계획" 참고 |
| **2** | Task 4-2 · API 협의 문서 v2.2 송부 | 🔄 진행 | Notion 게시 + Discord 공유 |
| **3** | ⭐ **`GET /api/businesses` — 행정동 코드 필터 필수** (BE) | ⏸️ 요청 예정 | 데이터는 DB에 적재·검증 완료(537,488건)이고 **엔드포인트만 없음**.<br>⚠️ **전체 조회는 프론트가 못 받는다** — 8/23 실측: 동 1개(1,265개) 25ms이나 서울 전체는 ~10초 추정. `district_code`로 잘라 주셔야 함 |
...
| **통합 Phase 2** — 라우팅 통합 + 지도 일원화 | 위와 동일 | 🔄 **2-A·2-B 완료 / 2-C·2-D 남음** |
...
└── widgets/                      # 17개
    ...
    │   ├── explore_panel.dart    # 선택 단계별 3상태 + 업종 드롭다운 + 블러 잠금
...
*FRONT-END WORKFLOW v5.1 · 사공민규 · 최종 수정: 2026.08.23*
