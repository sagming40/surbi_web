...
| 순위 | 작업 | 상태 | 비고 |
| --- | --- | --- | --- |
| **1** | **8/24 회의 지시 ③ — 통합 화면 ↔ Step 4 디자인 통일** | 🔜 **다음 차례** | 화면 크기·AppBar·배경색. **기준은 8/25에 만든 하단 시트** (`SurbiColors.primary` + `SurbiRadius.card`) |
| **2** | **8/24 회의 지시 ④ — Step 4 모바일 재구성** | ⏳ 대기 | SHAP/보고서/지원사업/체크리스트가 좁은 화면에서 두 화면으로 갈림. ③ 이후 착수 |
...
| **디자인 통일 + Step 4 모바일 재구성** | 8/24 회의 지시 ③④ | 🔜 **다음 차례** |
...
### 📐 하단 시트 3단계 (2026-08-24 회의 지시 ①② · 8/25 완료)
...
| **mid** | **계산** — min + 지역지표 3줄 + 구분선 + 업종 드롭다운 | 헤더 + 유동인구·상권변화·임대료 + 업종 드롭다운 |
| **max** | 고정 비율 `1.0` | 위 + 업종 지표 + `AI 창업 점수 보기` 버튼 |
...
**8/24 지시 ①②(손잡이·3단계 시트) 완료 / ③④(디자인 통일·Step 4 재구성) 미착수.**
경위는 DEVLOG 참조.
