...
### 📋 팀 정기회의 지시사항 (2026-07-13) — 전부 완료

| # | 지시 내용 | 성격 | 상태 |
| --- | --- | --- | --- |
| ① | 반응형 레이아웃을 웹 사용자 중심으로 재검토 | 제안 수준 | ⏳ 미착수 |
...
> ⚠️ **2026-08-16 발견** — 데스크탑·노트북 간 Flutter SDK 버전 차이로 빌드 실패 발생.
> 데스크탑(3.41.6, 5개월 전)이 노트북(3.44.6)보다 구버전이었고, `CupertinoPageTransitionsBuilder`가
> **material.dart의 암묵적 export에 의존**하고 있던 게 신버전에서 걸러짐 (커밋: `fix: main.dart Flutter SDK 버전 호환성 수정`).
> "데스크탑에서 되니까 맞다"고 가정하면 안 됨 — **구버전이 더 관대해서 우연히 통과했을 뿐일 수 있음.**
> 여러 환경에서 작업할 경우 **작업 시작 전 `flutter --version` 확인** 습관화 필요.

| 환경 | Flutter | Dart | 확인일 |
| --- | --- | --- | --- |
| 데스크탑 | 3.41.6 (2026-03-25 릴리즈) | 3.11.4 | 2026-08-16 |
| 노트북 | 3.44.6 (2026-07-08 릴리즈) | 3.12.2 | 2026-08-16 |

> ⚠️ **데스크탑이 5개월 더 구버전.** 노트북이 최신 SDK 기준으로 먼저 문제를 잡아낸 케이스.
> `CupertinoPageTransitionsBuilder`가 구버전에선 `material.dart`의 암묵적 export로 컴파일됐지만,
> 신버전에선 `cupertino.dart` 명시적 import가 필요해짐. 이런 "구버전에서만 우연히 되는" 코드가
