...
| 3 | Step 1 히트맵 — **점수 색칠 단계만 남음** | ⏳ 대기 | ①레이아웃 ②경계 렌더 ③점수 색칠 중 **②까지 완료(8/20)**. ③은 `fillColor`를 점수 기반 함수로 교체하는 작업 → Task 4-3에서 처리 |
| 4 | **Step 1 화면 재설계 (반응형 + 카테고리 칩 → 드롭다운)** | ⏳ 대기 | 7/13 회의 지시 ①(유일한 미착수)과 결합. `/select`는 maxWidth 500, `/map`은 infinity라 화면 전환 시 폭이 급변 |
...
| # | 지시 내용 | 성격 | 상태 |
| --- | --- | --- | --- |
| ① | 반응형 레이아웃을 웹 사용자 중심으로 재검토 | ~~제안 수준~~ → **필수 전제조건으로 승격** | ⏳ 미착수 — 히트맵을 하려면 지도가 커야 하고, 그러려면 이 항목이 먼저다. "다음 할 일" 4번 |
...
| 경계 렌더 성능 (실측) | ✅ **확인** (2026-08-20) — 구당 폴리곤 10~27개 / 좌표 109~241개 / **렌더 1~3ms**. 비용은 폴리곤 개수가 아니라 **좌표 개수**에 비례(약 100좌표당 1ms). GeoJSON 파싱은 앱당 1회(85ms) 후 캐시 |
| 렌더 범위 | **구 단위까지만** — 서울 전역 상시 렌더는 요구사항 아님(8/20 확정) |
...
| 1 | 히트맵 실데이터 (점수 기반 색칠) | 🔄 3단계 중 **②경계 렌더까지 완료(8/20)**. ③은 `drawGuBoundaries()`의 `fillColor`를 점수 기반 함수로 교체 → Task 4-3 |
| 1 | ~~구 전체 동 경계 렌더 (히트맵 밑그림)~~ | ✅ 완료 (2026-08-20) — `drawGuBoundaries()` · `showGuOnStep1()` |
| 1 | 카테고리 칩 10개 → 드롭다운 전환 + 반응형 재설계 | ⏳ "다음 할 일" 4번 — 히트맵의 전제조건 |
...
*FRONT-END WORKFLOW v4.4 · 사공민규 · 최종 수정: 2026.08.20*
