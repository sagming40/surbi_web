...
| ③ | Step 1 히트맵 영역에 **실제 지도** 렌더링 | 제안 | ✅ 완료 (마커 렌더링까지 포함, 제안 반영) |
| ④ | Step 3 **BottomSheet 재검토** (분리 or 대체) | 필수 | ✅ 완료 (카카오맵 CustomOverlay로 대체) |
| ⑤ | **Step 4-1을 부모 라우트로, 4-2·4-3·4-4를 자식으로 재편** | 필수 | ✅ 완료 |
...
| Step | 미완 항목 | 연결 Task |
| --- | --- | --- |
| 1 | 히트맵 실데이터 연동 (현재는 지도+구별 동 마커로 대체 — 실제 geom 기반 색칠 히트맵은 DB geom 컬럼 미적재로 여전히 블로킹) | Task 4-3 |
| 2 | 경쟁도 · 소비 패턴 차트 (유동인구 차트로 `fl_chart` 방식은 검증 완료) | Task 4-2 확정 후 판단 |
| 3 | 오버레이 카드 거리 표시 (사용자 위치 기반 계산) | Task 4-5 |
