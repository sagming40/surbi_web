...
> ※ v5.6 — 8/24 회의 지시 ④ Step 4 모바일 재구성: C안(아코디언) 구현 완료 — `feature/frontend-step4-mobile-accordion` 브랜치, D안(허브→상세)과 실물 비교 후 최종 채택 결정 예정 · **아직 main 미반영** (2026-08-27)
...
| 순위 | 작업 | 상태 | 비고 |
| --- | --- | --- | --- |
| **1** | **8/24 회의 지시 ④ — Step 4 모바일 재구성** | 🔄 **C안 구현 완료 (비교 대기)** | `feature/frontend-step4-mobile-accordion`에서 아코디언(C안)으로 구현 완료 — 잘림·분리 문제 해결. `feature/frontend-step4-mobile-hub-detail`(D안)도 구현 후 실물 비교하여 최종 채택안 결정 예정. **main 미반영** |
...
| **Step 4 모바일 재구성** | 8/24 회의 지시 ④ | 🔄 **C안 구현 완료, D안과 비교 후 결정** |
...
| ~~Step 4 모바일 재구성 방향~~ | 탭 재편(A) / 한 화면 스크롤(B) / 아코디언(C) / 허브→상세(D) 중 C·D 두 안을 브랜치로 구현해 비교하기로 함 | ✅ 방향 좁힘 (2026-08-27) — 최종 1개 확정은 비교 후 |
...
**8/24 지시 ①②(손잡이·3단계 시트) 완료 · ③(디자인 통일) 완료 / ④(Step 4 모바일 재구성) C안(아코디언) 구현 완료 — D안과 비교 후 최종 결정 예정.**
경위는 DEVLOG 참조.
...
└── widgets/                      # 18개
    ...
    ├── common/                   # 8개 (⭐ surbi_accordion_section 8/27 신설)
    │   ├── surbi_dropdown.dart   # OverlayEntry + LayerLink
    │   │                         #   openUpward · onMenuVisibilityChanged 옵션
    │   │                         #   collapsedHeight = 52 (시트 높이 계산이 함께 읽음)
    │   ├── surbi_accordion_section.dart # ⭐ 8/27 신설 — 접었다 펼치는 섹션
    │   │                         #   Visibility(maintainState: true)로 접혀도 State 안 죽음
    ...
    └── step4/                    # 10개 ⚠️ 폴더명만 Step 번호 잔존 — 정리 검토 대상
        ├── score_shell.dart      # ④ 허브 — LayoutBuilder 900px 분기(≥900 탭 / <900 아코디언, 8/27)
        ├── score_hub_panel.dart  # ⭐ 8/27 — ScoreOverviewHeader(게이지+예상성과) 분리, 넓은 화면은 SHAP까지 3장 그대로
        ├── score_gauge · shap_bar_chart(⚠️ B1)
...
| **4** | **모바일에서 SHAP/보고서/지원사업/체크리스트가 두 화면으로 갈림** | **8/24 지시 ④ — C안 구현 완료(브랜치), D안과 비교 후 결정** |
| 4 | ~~좁은 화면에서 "예상 성과" 카드가 탭바에 잘림~~ | ✅ 해소 (2026-08-27) — flex 4:6 분할 구조 자체를 걷어내 C·D 어느 쪽이 채택되든 재발 안 함 |
